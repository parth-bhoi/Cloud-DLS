...
import org.cloudbus.cloudsim.preCalculation;
	...
	protected void submitCloudlets() {
		...
		for (Cloudlet cloudlet : getCloudletList()) {
			...
			if (cloudlet.getVmId() == -1) {
				...
				int staticLevel = cloudlet.getClassType();
				double DL = Integer.MIN_VALUE;
				// Log.printConcatLine(machines.size(), " \n");
				double seta = 0;
				for(Vm vms : machines){
					double ext = preCalculation.getExpectedTime(vms.getId());
					double freeTime = preCalculation.getEta(vms.getId());
					Log.printConcatLine(staticLevel, " \n");
					double eta = (cloudlet.getCloudletLength()/(vms.getNumberOfPes()*vms.getMips())) + (cloudlet.getCloudletFileSize()/vms.getBw());
					double delta = ext - eta;
					double current = staticLevel - freeTime + delta;
					current = current*preCalculation.getTrust(vms);
				...
				preCalculation.settrust(vm, sign*1);
				if(randomNum == 0){
					// Random VM failed;
					continue;
				}
				preCalculation.setEta(seta, vm.getId());
			...
			cloudletsSubmitted++;
			vmIndex = (vmIndex + 1) % (machines.size());
